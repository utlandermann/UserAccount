...
Dim strDisplayName, strOffice, strPlace
Dim strItem, strDesc, strPhone, arrOtherPhone, strOtherPhone, strMobilePhone, strIPphone
Dim lngFlags, strEnabled, strFlags, objPwdLastSet, dtmPwdLastSet
Dim objShell, lngBiasKey, lngTZBias, k, arrAttrValues
Dim objFSO, logStream

' CSV fil håndtering
Set objFSO = CreateObject("scripting.filesystemobject")
Set logStream = objFSO.opentextfile("C:\domainusers7.csv", 8, True)
...
strAttributes = "distinguishedName,sAMAccountName,name,givenName,sn," _
    & "description,userAccountControl,pwdLastSet,physicalDeliveryOfficeName,l,telephoneNumber," _
    & "otherTelephone,mobile,ipphone"
...
' Output heading line.
logStream.writeline "Distinguished Name"",""sAMAccountName"",""Name"",""First Name""," _
    & """Last Name"",""Description"",""Account"",""Flags"",""Password Last Set""," _
    & """Office"",""Place"",""Telephone Number"",""Other Telephone Numbers""," _
    & """Mobile"",""IP Telephone"""
...
Do Until adoRecordset.EOF
    ...
    strOffice = adoRecordset.Fields("physicalDeliveryOfficeName").Value
    strDisplayName = adoRecordset.Fields("name").Value
    strPlace = adoRecordset.Fields("l").Value
    strPhone = adoRecordset.Fields("telephoneNumber").Value
    strMobilePhone = adoRecordset.Fields("mobile").Value
    strIPphone = adoRecordset.Fields("ipphone").Value
    ...
    strFlags = GetFlags(lngFlags)
		strEnabled = GetEnabled(lngFlags)

    ' Create array of string values to display.
    arrAttrValues = Array(strDN, strNTName,strDisplayName, strFirst, strLast, _
        strDesc, strEnabled, strFlags, CStr(dtmPwdLastSet), strOffice, strPlace, strPhone, _
        strOtherPhone, strMobilePhone, strIPphone)

    ' Display array of values in a comma delimited line, with each
    ' value enclosed in quotes.
    logStream.writeline CSVLine(arrAttrValues)
...
Function GetEnabled(ByVal lngFlag)
    ...
    If (lngFlag And ADS_UF_ACCOUNTDISABLE) <> 0 Then
        GetEnabled = GetEnabled & "," & "Account disabled"
    Else
    	  GetEnabled = GetEnabled & "," & "Account enabled"
    End If
    
    If (Len(GetEnabled) > 1) Then
        GetEnabled = Mid(GetEnabled, 2)
    End If
